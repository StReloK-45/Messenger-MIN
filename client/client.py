...
class ChatClient:
    ...
    def __init__(self):
        ...
        self.chat_port = 5555
        self.file_port = 5556
        self.message_history = []  # Только история ОБЩЕГО чата
        self.private_messages = {}  # {nick: [messages]} - история личных чатов
        self.private_files = {}     # {nick: [files]} - файлы личных чатов
        self.files_list = []        # Файлы ОБЩЕГО чата
        ...
        self.private_chats_list = set()  # Список ВСЕХ личных чатов
        ...
        self.load_config()
        self.load_chats_list()  # Загружаем сохранённые чаты
        self.ask_server_ip()
    ...
    def load_chats_list(self):
        """Загружает список всех личных чатов"""
    ...
    def save_chats_list(self):
        """Сохраняет список всех личных чатов"""
        chats_file = "private_chats.json"
        try:
    ...
    def load_private_messages(self, nick):
        """Загружает историю личных сообщений с конкретным пользователем"""
    ...
    def save_private_messages(self, nick):
        """Сохраняет историю личных сообщений с конкретным пользователем"""
    ...
    def show_full_history(self):
        """Показывает историю ТОЛЬКО общего чата"""
    ...
    def display_message(self, msg):
        """Отображает сообщение из ОБЩЕГО чата"""
        sender = msg.get('sender', '')
        text = msg.get('text', '')
        msg_time = msg.get('time', '')
        edited = msg.get('edited', False)
        ...
        if line.startswith("JSON_PAYLOAD:"):
            ...
            try:
                ...
                if msg_type == "history":
                    ...
                    # Добавляем в список чатов если нет
                    if target not in self.private_chats_list:
                        self.private_chats_list.add(target)
                        self.save_chats_list()
